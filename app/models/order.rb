class Order < ApplicationRecord
  ...
  def order_confirmation_to_owner_send
    if self.van.is_van_pro == false
      UserMailer.order_confirmation_to_owner_email(self.owner, self).deliver_now
    end
  end

  def admin_order_confirmation_van_private_send
    if self.van.is_van_pro == false
      AdminMailer.admin_order_confirmation_van_private_email(self.owner, self).deliver_now
    end
  end

  def admin_order_confirmation_van_pro_send
    if self.van.is_van_pro == true
      AdminMailer.admin_order_confirmation_van_pro_email(self.owner, self).deliver_now
    end
  end
end
